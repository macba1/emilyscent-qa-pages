...
exports.handler = async (event) => {
  ...
  // 4) If any are missing, serve the multi-step quiz form
  ...
  <style>
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    body {
      ...
      background:var(--bg); font-family:sans-serif; color:var(--text);
    }
    .quiz-wrapper {
...
<body>
  ...
  <script>
    ...
    nextBtn.onclick = () => {
      const field = steps[idx].querySelector('input,select');
  ...
  const question = `Which ${gender} perfume should I gift to ${target}, who is ${personality}, to evoke a ${mood} mood within a budget of ${budget}?`;
  ...
  // 7) Build affiliate link via concatenation to avoid backtick conflicts
  const linkId = "29d49a9185b1f48a905c292658d3be8a";
  const q = encodeURIComponent(perfumeName + " perfume");
  const amazonLink =
  ...
  await fetch(
    "https://script.google.com/macros/s/AKfycbz4CEzl7SBTPYVplSnu4lyfaSbB9TejKxl0Fi3He7WTR03Iqf_1AISKC6FM3H02Ne5ROw/exec",
    {
      ...
      body: JSON.stringify({
        slug: [target, gender, personality, mood, budget].join("-"),
        question,
        perfumeName,
        reason,
        amazonLink,
...
<head>
  ...
  <style>
    body { background:#F9F5F0; font-family:sans-serif; padding:2rem; }
    .card {
    ...
    .cta {
      ...
      padding:.75rem 1.5rem; background:#C62828; color:#fff;
...
<body>
  <div class="card">
    ...
    <a href="${amazonLink}" target="_blank" class="cta">Buy on Amazon</a>
  </div>
</body>
</html>`;
